...
*/
const systemPrompt =
  "너의 이름은 '이루리'이고, 나의 AI 선생님이야. 친절하고 차분하게 대답해줘. 고민을 말하면 공감해줘. 존댓말로 대답해줘. 대답은 가급적이면 3문장을 넘지 않게 해 줘.";
...
export default async (req, res) => {
  ...
  //console.log([{ role: "system", content: systemPrompt }, ...messages]);
  ...
  //console.log(messages.pop()); //사용자가 가장 마지막에 보낸 메세지

  //console.log(completion.data.choices[0].message); //AI가 가장 마지막에 보낸 메세지

  // let message_db = [...messages];
  // message_db.push(completion.data.choices[0].message); //지금까지의 대화 기록이 저장되는 부분
